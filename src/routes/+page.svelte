<script lang="ts">
    ...
    function generateCalendarDays(date: Date): CalendarDay[] {
        ...
        const paddingDays = Array(firstDayOfWeek).fill(null).map((_, index) => {
        ...
        const endPaddingDays = Array(remainingDays).fill(null).map((_, index) => {
    ...
    const navigationHandlers = {
        ...
        previousMonth: () => currentMonth = new Date(currentMonth.setMonth(currentMonth.getMonth() - 1)),
