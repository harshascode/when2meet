		...
		{:else}
			<div class="space-y-8">
				...
				<div class="">
					...
					<div class="shadow-xs rounded-lg border border-gray-200 bg-white p-4 sm:p-6">
						...
						<div class="relative overflow-x-auto pb-3">
							<div class="inline-block min-w-[600px] sm:min-w-full">
								<div class="overflow-hidden rounded-lg border border-gray-200">
									<div
										class="grid"
										style="grid-template-columns: 100px repeat({event?.dates?.length ||
											0}, minmax(60px, 1fr))"
									>
										<!-- Time Column (Sticky) -->
										...
										{#each event?.dates || [] as date}
											<div
												class="border-b border-l border-gray-200 bg-gray-50 p-2 text-center text-xs font-medium text-gray-700"
											>
												{formatDate(date)}
											</div>
										{/each}
										...
										{#each event?.timeSlots || [] as timeSlot}
											<div
												class="sticky left-0 z-10 border-r border-gray-200 bg-white p-2 text-xs text-gray-600"
											>
												{timeSlot}
											</div>
											{#each event?.dates || [] as date}
												<button
													type="button"
													class="h-8 border-b border-l border-gray-200 transition-colors duration-100 hover:opacity-90"
