<script lang="ts">
	...
	// Set current month/year based on today's date.
	let currentYear = $state(new Date().getFullYear());
	let currentMonth = $state(new Date().getMonth()); // 0 = January, etc.
	...
	function handleMouseUp() {
		...
		if (dragged) {
			// Update only cells that fall within the drag range.
			for (let row of grid) {
				for (let cell of row) {
					if (
			...
			// Force reactivity by reassigning the grid reference.
			grid = grid;
		}
		resetDrag();
	}
	...
	function toggleCell(cell: Cell) {
		if (dragged) return;
		// Find and update the cell in place.
		for (let row of grid) {
			for (let c of row) {
